...
import { AppHeroPhones } from "@/components/sections/app-monetization/hero-phones";
import { motionVariants } from "@/styles/animations";
import { appHero, appTrustBadges } from "@/data/app-monetization";

export function AppHero() {
  const KickerIcon = appHero.kickerIcon;
  const [headlineLead, headlineTail] = appHero.headline.split(appHero.headlineEmphasis);
          ...
          >
            ...
            <div className="animate-fade-up-1">
              <Heading as="h1" size="3xl" className="mt-6 text-balance">
                {headlineLead}
                <GradientText>{appHero.headlineEmphasis}</GradientText>
                {headlineTail}
              </Heading>
            </div>

            <p className="animate-fade-up-2 mt-6 max-w-xl text-lg text-muted-foreground">{appHero.description}</p>
            ...
            <div className="animate-fade-up-3 mt-8 flex flex-wrap items-center justify-center gap-4 lg:justify-start">
              <MagneticButton>
                <Button
                  ...
                  nativeButton={false}
                  render={
                    <Link href={appHero.primaryCta.href}>
                      {appHero.primaryCta.label}
                      <ArrowRight
              ...
              <MagneticButton strength={0.3}>
                <Button
                  ...
                  nativeButton={false}
                  render={
                    <Link href={appHero.secondaryCta.href}>
                      <Code2 className="size-4" aria-hidden="true" />
                      {appHero.secondaryCta.label}
                    </Link>
                  }
                />
              </MagneticButton>
            ...
            <div className="animate-fade-up-4 mt-10 flex flex-wrap items-center justify-center gap-x-4 gap-y-2 lg:justify-start">
              {appTrustBadges.map((badge, index) => (
                <Fragment key={badge}>
                  {index > 0 && (

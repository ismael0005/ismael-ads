...
import { CtvHeroStage } from "@/components/sections/ctv-monetization/hero-stage";
import { motionVariants } from "@/styles/animations";
import { ctvHero, ctvTrustBadges } from "@/data/ctv-monetization";

export function CtvHero() {
  const KickerIcon = ctvHero.kickerIcon;
  const [headlineLead, headlineTail] = ctvHero.headline.split(ctvHero.headlineEmphasis);
          ...
          >
            ...
            <div className="animate-fade-up-1">
              <Heading as="h1" size="3xl" className="mt-6 text-balance">
                {headlineLead}
                <GradientText>{ctvHero.headlineEmphasis}</GradientText>
                {headlineTail}
              </Heading>
            </div>

            <p className="animate-fade-up-2 mt-6 max-w-xl text-lg text-muted-foreground">{ctvHero.description}</p>
            ...
            <div className="animate-fade-up-3 mt-8 flex flex-wrap items-center justify-center gap-4 lg:justify-start">
              <MagneticButton>
                <Button
                  ...
                  nativeButton={false}
                  render={
                    <Link href={ctvHero.primaryCta.href}>
                      {ctvHero.primaryCta.label}
              ...
              <MagneticButton strength={0.3}>
                <Button
                  ...
                  nativeButton={false}
                  render={
                    <Link href={ctvHero.secondaryCta.href}>
                      <Sparkles className="size-4" aria-hidden="true" />
                      {ctvHero.secondaryCta.label}
                    </Link>
                  }
                />
              </MagneticButton>
            ...
            <div className="animate-fade-up-4 mt-10 flex flex-wrap items-center justify-center gap-x-4 gap-y-2 lg:justify-start">
              {ctvTrustBadges.map((badge, index) => (
                <Fragment key={badge}>

...
import { missionHero } from "@/data/my-mission";
import { MissionHeroStage } from "@/components/sections/my-mission/hero-stage";

export function MissionHero() {
  const KickerIcon = missionHero.kickerIcon;
  const [headlineLead, headlineTail] = missionHero.headline.split(missionHero.headlineEmphasis);

  return (
    <div className="relative isolate overflow-hidden text-foreground">
      <Container size="xl">
        {/* Stage-first on desktop — the AI-core orbit is this page's signature visual, and leading with it (rather than the text-first pattern every other hero uses) keeps the site from feeling machine-generated page to page. */}
          ...
          >
            ...
            <div className="animate-fade-up-1">
              <Heading as="h1" size="3xl" className="mt-6 text-balance">
                {headlineLead}
                <GradientText>{missionHero.headlineEmphasis}</GradientText>
                {headlineTail}
              </Heading>
            </div>

            <p className="animate-fade-up-2 mt-6 max-w-lg text-lg text-muted-foreground">{missionHero.description}</p>
            ...
            <div className="animate-fade-up-3 mt-8 flex flex-wrap items-center justify-center gap-4 lg:justify-start">
              <MagneticButton>
                <Button
                  ...
                  nativeButton={false}
                  render={
                    <Link href={missionHero.primaryCta.href}>
                      {missionHero.primaryCta.label}
              ...
              <MagneticButton strength={0.3}>
                <Button
                  ...
                  nativeButton={false}
                  render={
                    <Link href={missionHero.secondaryCta.href}>
                      <MessageCircle className="size-4" aria-hidden="true" />
                      {missionHero.secondaryCta.label}
                    </Link>
                  }
                />
              </MagneticButton>
            ...
            <div className="animate-fade-up-4 mt-8 flex flex-wrap items-center justify-center gap-x-4 gap-y-2 text-sm text-muted-foreground lg:justify-start">
              {missionHero.trustBadges.map((badge, index) => (
                <Fragment key={badge}>
